...
namespace Aspose.Tasks.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeTasksCloudContext.atcc = new AsposeTasksCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeTasksCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeTasksCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
